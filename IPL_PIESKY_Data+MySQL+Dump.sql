...
select ibgd.bidder_id,bidder_name, bid_status, (count(ibgd.BIDDER_ID)/NO_OF_BIDS)*100
from ipl_bidding_details ibgd
join ipl_bidder_details ibrd
on ibgd.BIDDER_ID=ibrd.BIDDER_ID
join ipl_bidder_points ibp
on ibp.BIDDER_ID=ibrd.BIDDER_ID
where ibgd.BID_STATUS='won'
group by ibgd.bidder_id, bid_status
order by (count(ibgd.BIDDER_ID)/NO_OF_BIDS)*100 desc;

#2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select ips.stadium_id,stadium_name,city,count(match_id) as number_of_matches
from ipl_stadium ips
join ipl_match_schedule ims
on ips.STADIUM_ID=ims.STADIUM_ID
group by ips.stadium_id,stadium_name,city;

#3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select ips.stadium_id,stadium_name,toss_winner,it.TEAM_NAME,(count(it.TEAM_ID)/its.matches_won)*100 as percentage_of_wins
from ipl_stadium ips
join ipl_match_schedule ims
on ips.STADIUM_ID=ims.STADIUM_ID
join ipl_match im
...
on its.TEAM_ID=im.TEAM_ID1
join ipl_team it
on it.TEAM_ID=its.TEAM_ID
group by ips.stadium_id, stadium_name,it.TEAM_NAME;

#4.	Show the total bids along with bid team and team name.
select ibrd.bidder_id,bidder_name,bid_team,team_name,sum(ibrp.no_of_bids) as total_bids
from ipl_bidder_details ibrd
join ipl_bidding_details ibgd
on ibrd.BIDDER_ID=ibgd.BIDDER_ID
join ipl_bidder_points ibrp
on ibrp.BIDDER_ID=ibgd.BIDDER_ID
cross join ipl_team it
on ibgd.BID_TEAM=it.TEAM_ID
group by bid_team,team_name;

#5.	Show the team id who won the match as per the win details.
select (im1.team_id1 and im2.team_id2) as team_id 
from ipl_match im1
join ipl_match im2
on im1.MATCH_WINNER=im2.MATCH_WINNER
where im1.MATCH_WINNER>0 and im2.WIN_DETAILS is not null
group by im1.team_id1,im2.team_id2;

          # or....#

select it.team_id,MATCH_WINNER,WIN_DETAILS
from ipl_team it
cross join ipl_match im
on it.TEAM_ID=im.TEAM_ID1
where im.MATCH_WINNER>0 and im.WIN_DETAILS is not null
group by it.TEAM_ID;
#6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select TEAM_NAME,sum(its.matches_played) as matches_played,sum(matches_won) as matches_won,sum(matches_lost) as matches_lost
from ipl_team_standings its
join ipl_team it
on its.TEAM_ID=it.TEAM_ID
group by its.matches_played,matches_won,matches_lost,team_name;
#7.	Display the bowlers for Mumbai Indians team.
select itp.PLAYER_ID,PLAYER_ROLE,REMARKS
from ipl_team_players itp
where PLAYER_ROLE like '%Bowler%' and remarks like '%mi%'
group by itp.PLAYER_ID,PLAYER_ROLE,REMARKS;

#8.	How many all-rounders are there in each team, Display the teams with more than 4 
	# all-rounder in descending order.
select itp.PLAYER_ID,PLAYER_ROLE,count(PLAYER_ROLE),REMARKS
from ipl_team_players itp
where (itp.PLAYER_ROLE like '%all%')
group by itp.PLAYER_ID,PLAYER_ROLE
order by count(PLAYER_ROLE) desc,remarks asc;
          # or..
select itp.player_id,PLAYER_ROLE, team_name
from ipl_team_players itp
join ipl_team it
